...
public class SJFNon {
   ...
   public Vector<Process> sortBurst(Vector<Process> arrP) { // should array enter to this fn be sorted by Arrival
      ...
      for(int i=0 ; i<arrP.size() ; i++){
          ...
          for(int j=i;j<arrP.size();j++)
          {
              ...
              if(arrP.get(j).getArrival()<=start && j!=i){
                  if(arrP.get(j).getBurst()<arrP.get(minIndex).getBurst()){
                     minIndex=j;
                     flag=true;
                  }
              }
